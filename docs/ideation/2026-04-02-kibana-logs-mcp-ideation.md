...
focus: mcp server for agent log search in basic-auth-protected Kibana, grounded on a realistic multi-source investigation workflow
---
...
The current repo is intentionally minimal and currently contains only `docs/sources/handoff.md` plus documentation artifacts. There is no existing implementation to constrain architecture, so the main grounding sources are the local handoff and the investigation requirements in this repo.

One repo-level note: `AGENTS.md` references `RTK.md`, but no such file exists in this workspace. The practical grounding sources for this ideation are therefore the handoff and the checked-in docs, not any missing repo convention document.
...
Observed constraints from the motivating investigation workflow:

- the agent must reconstruct a multi-source investigation around a precise trigger timestamp
- the key log surfaces are application logs, API logs, and structured metrics-style events such as `WORKFLOW_RELOAD_STATS` and `CACHE_REFRESH_PHASES`
- the agent must search by text markers such as `SERVICE_RELOAD`, then narrow by time window and identifiers like region, job id, or trace id when those fields exist
- the agent must compare before/during/after windows and extract concrete evidence for a verdict
- adjacent systems may matter to the full workflow, but they are outside the logs-only MCP scope requested here
...
**Rationale:** For a realistic investigation workflow, the agent needs a short path to the right sources. A configured catalog is both simpler and more reliable than dynamic full-environment discovery, while remaining general because the catalog is environment-driven rather than workflow-specific.
...
**Rationale:** This matches a realistic investigation workflow directly. The agent can start broad with counts or histograms, find the relevant execution window, and then switch to hits to inspect `SERVICE_RELOAD`, `WORKFLOW_RELOAD_STATS`, or `CACHE_REFRESH_PHASES` evidence without needing extra tools.
...
**Rationale:** The motivating workflow is fundamentally about reconstructing one incident across multiple log surfaces. If the response shape is correlation-first, the agent can align application events, API observations, and metric-like log lines without backend-specific parsing every time.
...
**Rationale:** Agents need repeatable behavior, not mystery translation. Echoing the compiled query makes investigations auditable and easier to debug when an agent narrows the search around a trigger timestamp.
...
**Rationale:** The motivating use case still points toward a two-tool default, but field ambiguity is the most likely reason that boundary would crack. Naming the escape hatch now keeps the architecture honest.
...
|---|------|-----------------|
| 1 | Dynamic crawl of every Kibana source in v1 | Too expensive and brittle for the simplest functional path; a configured source catalog is more reliable for the motivating use case while staying general. |
...
| 16 | Automatic environment detection | Nice-to-have, but explicit configuration is safer for operational environments. |
...
- 2026-04-02: Re-grounded on a realistic multi-source investigation workflow; shifted recommendation toward a configured source catalog plus a multi-mode general query tool as the highest-probability simple v1; 6 survivors kept
