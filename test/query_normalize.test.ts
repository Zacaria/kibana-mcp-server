...
describe("normalizeQueryResponse", () => {
  it("normalizes hits responses", () => {
      ...
      sourceQueries: [
        {
          source,
          resolvedFilters: [
            { field: "productId", resolved_field: "productId", value: "123" }
          ],
    ...
    expect(result.hits?.[0]?.summary).toContain("reload completed");
    expect(result.hits?.[0]?.selected_fields.productId).toBe("123");
  ...
  it("normalizes grouped counts", () => {
    ...
    const result = normalizeQueryResponse(plan, [
      {
        ...
        rawResponse: {
          aggregations: {
            groups: {
              buckets: [
                { key: "MEMOIZE_V3:PRODUCT_OPENING_DATES_V3", doc_count: 2 }
              ]
            }
          }
        }
    ...
    expect(result.groups?.[0]?.buckets[0]?.key).toBe("MEMOIZE_V3:PRODUCT_OPENING_DATES_V3");
  });
  ...
  it("globally sorts and truncates multi-source hits by the requested sort field", () => {
    ...
    expect(result.hits?.map((hit) => hit.document_id)).toEqual(["api-1", "consumer-1"]);
